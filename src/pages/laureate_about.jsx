import { useState, useEffect } from "react";
import { Link } from "react-router-dom";

const LaureateAbout = () => {
  const [loaded, setLoaded] = useState(false);
  const [hoverCTA, setHoverCTA] = useState(false);
  const [menuOpen, setMenuOpen] = useState(false);

  useEffect(() => {
    setTimeout(() => setLoaded(true), 100);
  }, []);

  const fadeIn = (delay) => ({
    opacity: loaded ? 1 : 0,
    transform: loaded ? "translateY(0)" : "translateY(16px)",
    transition: `opacity 0.9s ease ${delay}s, transform 0.9s ease ${delay}s`,
  });

  const sectionStyle = {
    maxWidth: "700px",
    margin: "0 auto",
    padding: "0 48px",
  };

  const labelStyle = {
    fontSize: "13px",
    letterSpacing: "4px",
    textTransform: "uppercase",
    color: "#9E7C4B",
    marginBottom: "24px",
    textAlign: "center",
  };

  const bodyStyle = {
    fontSize: "17px",
    lineHeight: 2.0,
    color: "#5A6270",
    margin: "0 0 20px",
  };

  const emphStyle = {
    fontSize: "17px",
    lineHeight: 2.0,
    color: "#1A2332",
    margin: "0 0 20px",
  };

  return (
    <div style={{
      minHeight: "100vh",
      background: "#FAFAF8",
      color: "#1A2332",
      fontFamily: "'Georgia', 'Times New Roman', serif",
      overflowX: "hidden",
    }}>
      {/* ── NAV ── */}
      <nav style={{
        padding: "28px 48px",
        position: "relative",
        display: "flex",
        justifyContent: "space-between",
        alignItems: "center",
        ...fadeIn(0.1),
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          <Link to="/"><img src="data:image/png;base64," alt="Laureate Edition" style={{ height: "70px", width: "auto" }}/></Link>
        </div>
        <div style={{ display: "flex", gap: "36px", fontSize: "13px", letterSpacing: "2px", textTransform: "uppercase" }} className="le-desktop-nav">
          <Link to="/" style={{ color: "#8C8C8C", textDecoration: "none" }}>Home</Link>
          <Link to="/about" style={{ color: "#1A2332", textDecoration: "none" }}>About</Link>
          <Link to="/agencies" style={{ color: "#8C8C8C", textDecoration: "none" }}>For Agencies</Link>
          <Link to="/contact" style={{ color: "#8C8C8C", textDecoration: "none" }}>Contact</Link>
        </div>
        <button
        onClick={() => setMenuOpen(!menuOpen)}
        style={{ display: "none", flexDirection: "column", gap: "5px", background: "none", border: "none", cursor: "pointer", padding: "4px" }}
        className="le-hamburger"
      >
        <span style={{ display: "block", width: "24px", height: "2px", background: "#1A2332" }} />
        <span style={{ display: "block", width: "24px", height: "2px", background: "#1A2332" }} />
        <span style={{ display: "block", width: "24px", height: "2px", background: "#1A2332" }} />
      </button>
      {menuOpen && (
        <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "#FAFAF8", padding: "24px 48px", display: "flex", flexDirection: "column", gap: "20px", fontSize: "13px", letterSpacing: "2px", textTransform: "uppercase", zIndex: 100, boxShadow: "0 4px 12px rgba(0,0,0,0.08)" }}>
          <Link to="/" onClick={() => setMenuOpen(false)} style={{ color: "#8C8C8C", textDecoration: "none" }}>Home</Link>
          <Link to="/about" onClick={() => setMenuOpen(false)} style={{ color: "#8C8C8C", textDecoration: "none" }}>About</Link>
          <Link to="/agencies" onClick={() => setMenuOpen(false)} style={{ color: "#8C8C8C", textDecoration: "none" }}>For Agencies</Link>
          <Link to="/contact" onClick={() => setMenuOpen(false)} style={{ color: "#8C8C8C", textDecoration: "none" }}>Contact</Link>
        </div>
      )}
      <style>{`
        .le-hamburger { display: none !important; }
        @media (max-width: 768px) {
          .le-desktop-nav { display: none !important; }
          .le-hamburger { display: flex !important; }
        }
      `}</style>
    </nav>

      {/* ── PAGE TITLE ── */}
      <section style={{
        textAlign: "center",
        padding: "80px 48px 60px",
        ...fadeIn(0.3),
      }}>
        <h1 style={{
          fontSize: "36px",
          fontWeight: "normal",
          letterSpacing: "1px",
          color: "#1A2332",
          margin: "0 0 16px",
        }}>
          About Laureate Edition
        </h1>
        <div style={{
          width: "40px",
          height: "1px",
          background: "#9E7C4B",
          margin: "0 auto",
          opacity: 0.4,
        }} />
      </section>

      {/* ── THE OBSERVATION ── */}
      <section style={{ ...sectionStyle, paddingBottom: "64px", ...fadeIn(0.5) }}>
        <p style={labelStyle}>The Observation</p>

        <p style={bodyStyle}>
          Over time, we began to notice something.
        </p>

        <p style={bodyStyle}>
          The people who most shaped how organizations actually work,
          how decisions were made, how teams were built, how professionals
          practiced their craft, were rarely the ones writing books.
        </p>

        <p style={bodyStyle}>
          Their ideas were everywhere. In boardrooms and classrooms. In
          training programs and professional certifications. In the language
          used by people who had never met them. Colleagues referenced
          their frameworks. Students repeated their concepts. Entire
          disciplines formed around ways of thinking they had originated.
        </p>

        <p style={emphStyle}>
          Yet most of it existed only in presentations, conversations,
          and scattered writing. Never assembled into a single,
          coherent work.
        </p>

        <p style={bodyStyle}>
          Not because the ideas lacked substance. Because the kind of
          professional life that produces genuine expertise rarely leaves
          the uninterrupted time required to write a traditional manuscript.
        </p>
      </section>

      {/* ── THE GAP ── */}
      <section style={{
        background: "#1A2332",
        padding: "64px 48px",
      }}>
        <div style={{ maxWidth: "700px", margin: "0 auto", ...fadeIn(0.7) }}>
          <p style={{ ...labelStyle, color: "#9E7C4B" }}>The Gap</p>

          <p style={{
            fontSize: "20px",
            lineHeight: 1.9,
            color: "#E8E4DF",
            margin: "0 0 28px",
            textAlign: "center",
          }}>
            Traditional publishing depends on authors who can step away
            and write. Self-publishing depends on authors who want to
            manage the process themselves.
          </p>

          <p style={{
            fontSize: "18px",
            lineHeight: 1.9,
            color: "#8899AA",
            margin: "0 0 32px",
            textAlign: "center",
            maxWidth: "600px",
            marginLeft: "auto",
            marginRight: "auto",
          }}>
            Most experienced professionals are neither unwilling nor
            uninterested. They are simply still engaged in the work that
            created the knowledge in the first place.
          </p>

          <p style={{
            fontSize: "18px",
            lineHeight: 1.9,
            color: "#8899AA",
            margin: "0 0 36px",
            textAlign: "center",
            maxWidth: "600px",
            marginLeft: "auto",
            marginRight: "auto",
          }}>
            We kept meeting accomplished leaders and practitioners who
            were regularly asked the same question: whether they had
            ever written a book. The answer was always the same.
          </p>

          <p style={{
            fontSize: "26px",
            color: "#E8E4DF",
            textAlign: "center",
            fontStyle: "italic",
            margin: "0 0 36px",
            letterSpacing: "0.5px",
          }}>
            "Not yet."
          </p>

          <p style={{
            fontSize: "16px",
            color: "#9E7C4B",
            textAlign: "center",
            letterSpacing: "0.5px",
          }}>
            Laureate Edition was created to close that gap.
          </p>
        </div>
      </section>

      {/* ── THE MODEL ── */}
      <section style={{ ...sectionStyle, paddingTop: "64px", paddingBottom: "64px", ...fadeIn(0.9) }}>
        <p style={labelStyle}>The Model</p>

        <p style={bodyStyle}>
          Laureate Edition operates less like a traditional writing process
          and more like an editorial development partnership.
        </p>

        <p style={bodyStyle}>
          Rather than asking an author to produce a manuscript alone, we
          work through a structured series of guided interviews,
          collaborative sessions, and iterative editorial development.
          Professional writers and experienced editors shape the material
          into a coherent manuscript while the author directs the ideas,
          the judgments, and the perspective.
        </p>

        <p style={emphStyle}>
          The objective is not to replace the author's voice. It is to
          capture it accurately.
        </p>

        <p style={bodyStyle}>
          We focus on clarity, structure, and permanence. The goal is
          to assemble a body of thinking into a work that reads as
          intentional rather than accumulated.
        </p>

        {/* Co-author model — present but not prominent */}
        <div style={{
          borderTop: "1px solid #E8E5E0",
          borderBottom: "1px solid #E8E5E0",
          padding: "28px 0",
          margin: "32px 0",
        }}>
          <p style={bodyStyle}>
            For select projects, we invite a small group of accomplished
            professionals to contribute chapters that complement and
            expand the central work. These contributors are chosen for
            their direct experience with the themes the book explores,
            and their material is developed through the same guided
            editorial process. The result is a finished book that reflects
            both established expertise and contemporary professional
            practice.
          </p>
        </div>
      </section>

      {/* ── EDITORIAL STEWARDSHIP ── */}
      <section style={{
        background: "#F5F3EF",
        padding: "64px 48px",
      }}>
        <div style={{ maxWidth: "700px", margin: "0 auto", ...fadeIn(1.0) }}>
          <p style={labelStyle}>Editorial Stewardship</p>

          <p style={bodyStyle}>
            Every project is developed on a defined timeline with clear
            editorial milestones. Our team oversees structure, manuscript
            development, design, production, and global distribution
            so the author can remain focused on their work while still
            guiding the intellectual direction of the book.
          </p>

          <p style={bodyStyle}>
            We develop a limited number of titles each year. The standard
            is not speed or volume. It is a finished work that feels
            considered, durable, and faithful to the author's contribution
            to their field.
          </p>
        </div>
      </section>

      {/* ── THE ORIGIN ── */}
      <section style={{ ...sectionStyle, paddingTop: "64px", paddingBottom: "64px", ...fadeIn(1.1) }}>
        <p style={labelStyle}>The Origin</p>

        <p style={bodyStyle}>
          Laureate Edition is led by a team of publishing professionals
          who have guided books to market across business, leadership,
          professional practice, and public life. Their authors have
          included business leaders, senior military officers, Olympic
          gold medalists, faith leaders, and celebrated restaurateurs,
          among others.
        </p>

        <p style={bodyStyle}>
          Over the years, we repeatedly encountered the same situation:
          individuals whose ideas had shaped entire fields, whose
          frameworks were taught by others, whose influence was
          unmistakable. And yet none of it had ever been captured
          in a book.
        </p>

        <p style={emphStyle}>
          We created this imprint to provide a publishing structure
          built for experienced professionals. One that respects both
          their time and the seriousness of attaching their name
          to a published work.
        </p>
      </section>

      {/* ── OUR VIEW ── */}
      <section style={{
        background: "#1A2332",
        padding: "64px 48px",
      }}>
        <div style={{
          maxWidth: "620px",
          margin: "0 auto",
          textAlign: "center",
          ...fadeIn(1.2),
        }}>
          <p style={{ ...labelStyle, color: "#9E7C4B" }}>Our View</p>

          <p style={{
            fontSize: "20px",
            lineHeight: 1.8,
            color: "#E8E4DF",
            margin: "0 0 28px",
          }}>
            A book should not exist simply because someone wanted
            to write one.
          </p>

          <p style={{
            fontSize: "20px",
            lineHeight: 1.8,
            color: "#E8E4DF",
            margin: "0 0 36px",
          }}>
            It should exist because knowledge that shaped real decisions
            deserves a permanent form.
          </p>

          <div style={{
            width: "40px",
            height: "1px",
            background: "#9E7C4B",
            margin: "0 auto 36px",
            opacity: 0.4,
          }} />

          <p style={{
            fontSize: "16px",
            lineHeight: 1.8,
            color: "#8899AA",
            margin: "0 0 8px",
          }}>
            Laureate Edition is for professionals whose influence has
            already been demonstrated in practice, and who are ready to
            formalize their thinking into a definitive work.
          </p>

          <p style={{
            fontSize: "14px",
            color: "#8899AA",
            fontStyle: "italic",
            margin: "0",
            letterSpacing: "0.5px",
          }}>
            We do not operate as an open submission press.
            Projects begin with a conversation.
          </p>
        </div>
      </section>

      {/* ── CTA ── */}
      <section id="contact" style={{
        padding: "64px 48px 80px",
        textAlign: "center",
        ...fadeIn(1.3),
      }}>
        <p style={{
          fontSize: "22px",
          color: "#1A2332",
          lineHeight: 1.6,
          maxWidth: "500px",
          margin: "0 auto 12px",
          fontWeight: "normal",
        }}>
          If your work has shaped a field and you're<br />
          considering a definitive publication —<br />
          we can explore fit.
        </p>

        <p style={{
          fontSize: "15px",
          color: "#8C8C8C",
          marginBottom: "36px",
        }}>
          A short call to determine whether this is the right partnership.
        </p>

        <a
          href="mailto:hello@laureateedition.com"
          onMouseEnter={() => setHoverCTA(true)}
          onMouseLeave={() => setHoverCTA(false)}
          style={{
            display: "inline-block",
            padding: "14px 40px",
            border: `1px solid ${hoverCTA ? "#9E7C4B" : "#1A2332"}`,
            color: hoverCTA ? "#9E7C4B" : "#1A2332",
            fontSize: "13px",
            letterSpacing: "3px",
            textTransform: "uppercase",
            textDecoration: "none",
            fontFamily: "inherit",
            transition: "all 0.3s ease",
            cursor: "pointer",
          }}
        >
          Discuss a potential project
        </a>

        <p style={{
          fontSize: "13px",
          color: "#BCBCBC",
          marginTop: "20px",
        }}>
          hello@laureateedition.com
        </p>
      </section>

      {/* ── FOOTER ── */}
      <footer style={{
        padding: "40px 48px",
        borderTop: "1px solid #E8E5E0",
        display: "flex",
        justifyContent: "space-between",
        alignItems: "center",
        flexWrap: "wrap",
        gap: "12px",
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          <Link to="/"><img src="data:image/png;base64," alt="Laureate Edition" style={{ height: "40px", width: "auto" }}/></Link>
        </div>
        <span style={{ fontSize: "11px", color: "#BCBCBC" }}>
          &copy; 2026 Laureate Edition. All rights reserved.
        </span>
      </footer>
    </div>
  );
};

export default LaureateAbout;
